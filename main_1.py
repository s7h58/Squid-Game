...
heart_image = pygame.image.load("image/heart.png").convert_alpha()  # 하트 이미지
...
def draw_lives():
    for i in range(fighter.lives):
        screen.blit(heart_image, (560 - i * 40, 10))  # 오른쪽 정렬
...
while 1:
    ...
    for badguy in badguys[:]:
        if fighter.hit_by(badguy):
            ...
            if fighter.lives <= 0:
                game_over1 = True
                game_over_reason1 = "Game Over"
    if game_over1:
        screen.blit(GAME_OVER, (160, 200))
        pygame.display.flip()
        pygame.time.delay(2000)
        game_over_reason1 = False
